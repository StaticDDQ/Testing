...
public class TimeControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.LeftShift) && Input.GetKeyDown(KeyCode.C) && !underEffect && !GameProgress.GetPaused())
        {
            SpeedTime();
        } else if(Input.GetKeyDown(KeyCode.C) && !underEffect && !GameProgress.GetPaused())
        {
            SlowTime();
        }
    }
